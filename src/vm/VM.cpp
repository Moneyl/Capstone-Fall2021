...
#define STACK_OVERFLOW_CHECK() if (SP <= ProgramSize() + VariablesSize())\
...
Result<void, VMError> VM::Run(Span<Instruction> program)
{
    ...
    memcpy(&Memory[0], program.data(), program.size_bytes());
    _programSize = program.size_bytes();

    //Run program
    while (PC < _programSize)
    {
        ...
        u16 regB = instruction.OpRegisterRegister.RegA;
...
{
    if (SP <= ProgramSize() + VariablesSize()) //Prevent stack from growing into variable/program memory
